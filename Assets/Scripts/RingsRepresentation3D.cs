...
using System.Linq;
using UnityEngine;
class RingsRepresentation3D: MonoBehaviour{
    ...
    public static GameObject RenderCircles(string name, Puzzle p) {
        ...
        List<int> matChoice = new List<int>() { 0, 1, 0, 2 };
        
        // define the parcours /// \todo Geometry3D.Axis { x, y, z, none }
        // certain trail will be used twice as we have to align the sphere mesh
        // in that case we do a 90° rotation instead of a full one. 
        ...
        for (int i = 0; i < stickerChoice.Count; i++) {
            ...
            for (int j = 0; j < 8; j++) {
                ...
                switch (j) {
                    // rotation j = 2 => reajust the sticker position to get on the right circle
                    case 2:
                        stickerReference = RingsRepresentation3D.TraverseAxis(stickerReference, tempsticker, vertices,
                    ...
                    default:
                        ...
                        for (int k = 0; k < 90; k++) {
                            ...
                            stickerReference = RingsRepresentation3D.TraverseAxis(stickerReference, tempsticker, 
                                vertices, rotationAxes[j].Item1, rotationAxes[j].Item2, 4f);
                        }
                        Mesh circleMesh = RingsRepresentation3D.CreateCircleMesh(vertices);
                        GameObject circle = RingsRepresentation3D.CreateCircle(circleMesh, matChoice[j], i);
    ...
    static Mesh CreateCircleMesh(List<Vector3> vertices) {
        /// \todo into RingsRepresentation3D.cs
        Mesh mesh = new Mesh();
    ...
    {
        /// \todo remove GameManager3D "attributes"?
        ...
        return Geometry3D.RotationMatrix((Geometry.Axis)axis1, (Geometry.Axis)axis2, angle) * stickers;
    }
    ...
    {
        /// \todo remove GameManager3D "attributes"?
        sticker.transform.position = Geometry3D.Projection3DTo2D(GameManager3D3D.cameraRotation * stickers);
        if (makeVertices) {
            float vertexX = trailWidth * Mathf.Sin(angle);
            float vertexY = trailWidth * Mathf.Sin(angle);
            float vertexZ = trailWidth * Mathf.Cos(angle);
