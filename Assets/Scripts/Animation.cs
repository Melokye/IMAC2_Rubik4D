...
class Animation {
    ...
    /// </summary>
    public static List<List<Vector4>> DefineTargets(Puzzle p, SelectCell selectedCell, Geometry.Axis begin, Geometry.Axis end) {
        ...
        for (int i = 0; i < p.NbCells(); i++) {
            ...
            for (int j = 0; j < p.NbStickers(i); j++) {
                if (p.whosGunnaRotate(selectedCell)[i][j]) {
